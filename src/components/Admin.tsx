...
function rowImages(row: { image_url?: string; image_urls?: string[] }) {
...
export default function Admin() {
  ...
  async function loadData(nextPassword = password) {
    ...
    try {
      ...
      setData({ ...payload, market_favorites: payload.market_favorites || [], market_items: payload.market_items || [] });
      setOk(true);
  ...
  return (
    <div className="p-4 md:p-8 min-h-screen bg-paper-texture">
      <div className="max-w-6xl mx-auto space-y-6">
                    ...
                    {notes.map((note) => {
                      ...
                      return (
                        <div key={note.id} className="text-xs text-slate-600 border-b last:border-b-0 py-2 space-y-2">
                          ...
                          <ImageGrid images={parsed.images} />
                        </div>
                      );
                    })}
        ...
        <DataPanel title="会员注册数据">
          <Table headers={['姓名', '电话', '登录方式', '注册时间']}>
            {(data?.members || []).map((item) => (
              <tr key={item.id} className="border-b last:border-b-0">
                <Td>{item.name}</Td>
                <Td>{item.phone}</Td>
                <Td>{item.login_type === 'wechat' ? '微信' : '手机'}</Td>
                <Td>{new Date(item.created_at).toLocaleString('zh-CN')}</Td>
              </tr>
            ))}
          </Table>
          {data?.members.length === 0 && <Empty />}
        </DataPanel>

        <DataPanel title="报名名单">
          <Table headers={['姓名', '电话', '课程', '状态', '提交时间']}>
            ...
            {(data?.notes || []).map((note) => {
              ...
              return (
                <div key={note.id} className="border border-primary/10 rounded-xl p-4 bg-white">
                  ...
                  <p className="text-xs text-slate-400 mt-1">会员ID：{note.user_id}</p>
                  <p className="text-sm text-slate-600 whitespace-pre-wrap mt-2">{parsed.text}</p>
                  <ImageGrid images={parsed.images} />
                </div>
              );
            })}
        ...
        <DataPanel title="雅集收藏数据">
          <div className="grid md:grid-cols-2 gap-3">
            {(data?.market_favorites || []).map((item) => (
              <div key={item.id} className="border border-primary/10 rounded-xl p-4 bg-white">
                <div className="flex gap-4">
                  <div className="min-w-0">
                    <h3 className="font-bold text-primary">{item.name}</h3>
                    <p className="text-xs text-slate-500 mt-1">会员ID：{item.user_id}</p>
                    <p className="text-xs text-slate-500 mt-1">¥ {item.price} · {new Date(item.created_at).toLocaleString('zh-CN')}</p>
                  </div>
                </div>
                <ImageGrid images={rowImages(item)} />
              </div>
          ...
          {data?.market_favorites.length === 0 && <Empty />}
        </DataPanel>
            ...
            {(data?.courses || []).map((course) => (
              <div key={course.id} className="border border-primary/10 rounded-xl p-4 bg-white">
                ...
                <p className="text-xs text-slate-500 mt-1">
                  {course.date_info} · {course.location}
                </p>
              </div>
            ))}
          </div>
