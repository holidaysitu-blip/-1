...
import { Home, School, LayoutGrid, User, Bot, Sprout, Map } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex flex-col bg-paper-texture">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white/90 backdrop-blur-md border-t border-primary/10 flex justify-around items-center px-4 pb-safe z-50">
        ...
        <NavButton to="/discovery" icon={<Map size={24} />} label="章园" />
    ...
    >
      {({ isActive }) => (
        <>
          <motion.div
            whileTap={{ scale: 0.9 }}
            className={cn("flex items-center justify-center p-1", isActive && "fill-icon")}
          >
            {icon}
          </motion.div>
